...
// Create User
export async function createUser(email, password, username, expoPushToken) {
...
export const getUsersWithTokens = async () => {
  try {
    const response = await databases.listDocuments(
      appwriteConfig.databaseId,
      appwriteConfig.userCollectionId,
      [Query.notEqual('token', '')] // Fetch users where token is not empty
    );

    // Extract tokens from user documents
